...
$sql_motorcycles = "SELECT motorcycles.id AS id, model, year, engine_cc, engine_hp, category_id, categories.id AS category_name, categories.name AS category_name 
  FROM motorcycles, categories 
  WHERE manufacturer_id = " . $manufacturer_id . " AND manufacturer_id = categories.id";
    ...
    <?php } else { ?>
      <table id="motorcycles">
        ...
        while($motorcycle = $motorcycles->fetch_assoc()) {
          ...
          $catagory_id = $motorcycle['category_id'];
          $catagory_name = $motorcycle['category_name'];
        ?>
          <tr>
            ...
            <td><a href="/catagories/show.php?id=<?= $catagory_id ?>"><?= $catagory_name ?></a></td>
            <td><?= $cc ?>cc, <?= $hp ?>hp</td>
          </tr>
        <?php } // end while ?>
        </tbody>
